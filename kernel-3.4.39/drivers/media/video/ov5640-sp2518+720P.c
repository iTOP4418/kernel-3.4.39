...
//add by dg for power 2015-09-12
static int s5p4418_ov5640_power(int flag)
{

    int err;
    printk("ov5640 sensor is power %s\n",flag == 1 ?"on":"off");
    ...
    if(1 == flag)
    {
        back_camera_power_enable(true);

    }else
    {
        back_camera_power_enable(false);
    }
...
static int ov5640_init(struct v4l2_subdev *sd)
{
        ...
        printk("ov5640 device probed success on %s.......\n",__func__);
...
static int sp2518_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //add by dg
    camera_common_vin_setup_io(0,true);

    msleep(100);

    s5p4418_ov5640_power(1);
