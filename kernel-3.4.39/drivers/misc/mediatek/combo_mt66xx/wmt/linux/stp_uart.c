...
unsigned int gUARTDbgLevel = UART_LOG_INFO;
//unsigned int gUARTDbgLevel = UART_LOG_LOUD;

#define UART_DBG_FUNC(fmt, arg...)    if(gUARTDbgLevel >= UART_LOG_DBG){  printk(KERN_DEBUG PFX "%s: "  fmt, __FUNCTION__ ,##arg);}
#define UART_INFO_FUNC(fmt, arg...)   if(gUARTDbgLevel >= UART_LOG_INFO){ printk(PFX "%s: "  fmt, __FUNCTION__ ,##arg);}
#define UART_WARN_FUNC(fmt, arg...)   if(gUARTDbgLevel >= UART_LOG_WARN){ printk(PFX "%s: "  fmt, __FUNCTION__ ,##arg);}
#define UART_ERR_FUNC(fmt, arg...)    if(gUARTDbgLevel >= UART_LOG_ERR){  printk(PFX "%s: "   fmt, __FUNCTION__ ,##arg);}
#define UART_TRC_FUNC(f)              if(gUARTDbgLevel >= UART_LOG_DBG){  printk(KERN_DEBUG PFX "<%s> <%d>\n", __FUNCTION__, __LINE__);}
...
////    spin_lock_irqsave(&buf_lock, flags);
    ...
    if(len > 0 && len < MAX_PACKET_ALLOWED)
    {
        ...
        */
        ...
        while(written != len)
        {
             UART_ERR_FUNC("WARN(i-%d):[pid(%d)(%s)]tty-ops->write FAIL!len(%d)wr(%d)wr_i(%d)rd_i(%d)\n\r", i, current->pid, current->comm, len, written, wr_idx, rd_idx);
             msleep(2);
             written += tty->ops->write(tty, &tx_buf[rd_idx+written], len-written);
            // UART_ERR_FUNC("WARN(i-%d):[pid(%d)(%s)]tty-ops->write FAIL!len(%d)wr(%d)wr_i(%d)rd_i(%d)\n\r", i, current->pid, current->comm, len, written, wr_idx, rd_idx);
        }
        #else
        if(written != len) {
