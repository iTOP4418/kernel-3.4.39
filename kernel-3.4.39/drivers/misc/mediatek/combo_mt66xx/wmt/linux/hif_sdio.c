...
#ifdef MTK_CMB_SDIO_EINT
extern struct _mtk_sdio_eint mtk_sdio_eint;
extern unsigned long ext_irq_flag; 
#endif
...
#ifdef MTK_CMB_SDIO_EINT
                    ret = mtk_hif_sdio_claim_irq(g_hif_sdio_probed_func_list[j].func, hif_sdio_irq);
#else
                    ret = sdio_claim_irq(g_hif_sdio_probed_func_list[j].func, hif_sdio_irq);
#endif
					if(ret)
                    	HIF_SDIO_INFO_FUNC("sdio_claim_irq error = %d\n", ret);
    ...
    )
{
    ...
    for ( i=0; i<ARRAY_SIZE(mtk_sdio_id_tbl); i++ )
    {
...
static int hif_sdio_suspend (struct device *dev)
{
    ...
    sdio_claim_host(func);
#ifdef MTK_CMB_SDIO_EINT
	set_bit(IN_SUSPEND, &ext_irq_flag);
#endif
...
#ifdef MTK_CMB_SDIO_EINT
	clear_bit(IN_SUSPEND, &ext_irq_flag);
#endif
    func = dev_to_sdio_func(dev);
	sdio_release_host(func);
...
#ifdef MTK_CMB_SDIO_EINT
	smp_rmb();
	if (test_and_clear_bit(IRQ_IN_SUSPEND, &ext_irq_flag)) {
    	HIF_SDIO_DBG_FUNC("intr before resume, handl it\n");
    ...
    )
{
    ...
    sdio_claim_host(func);
#ifdef MTK_CMB_SDIO_EINT
    ret = mtk_hif_sdio_claim_irq(func, hif_sdio_irq);
#else
...
stp_on_exist:
    ...
    if ( (clt_index = g_hif_sdio_probed_func_list[probe_index].clt_idx) >= 0 )    /* the function has been registered */
    {
        ...
        if (ret) {
            ...
            sdio_claim_host(func);
#ifdef MTK_CMB_SDIO_EINT
            ret2 = mtk_hif_sdio_release_irq(func);
#else
...
stp_off_exist:
    ...
    sdio_claim_host(func);
#ifdef MTK_CMB_SDIO_EINT
    ret = mtk_hif_sdio_release_irq(func);
#else
...
INT32 mtk_wcn_hif_sdio_is_card_removed(void)
{
	INT32 probe_index = -1;

    if ( (probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x020A, 1)) >= 0 ||
		 (probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x020C, 2)) >= 0 || 
		 (probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x6628, 1)) >= 0 ||
		 (probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x018A, 1)) >= 0 ||
		 (probe_index = hif_sdio_find_probed_list_index_by_id_func(0x037A, 0x018C, 2)) >= 0 ) 
	{
       	return 0;
    } else {
		return 1;
...
EXPORT_SYMBOL(mtk_wcn_hif_sdio_is_card_removed);
