...
#if 0
	//#define TP_IOCTL        IMX_GPIO_NR(6, 15)//NANDF_CS2 CABC_EN0
	#define TP_IOCTL	IMX_GPIO_NR(6, 16)//NANDF_CS3 CABC_EN1

        printk("==%s: reset==\n", __FUNCTION__);
        ret = gpio_request(TP_IOCTL, "TP_IOCTL");
        if (ret) {
                gpio_free(TP_IOCTL);
                ...
                if(ret)
                {
                        printk("ft5xox: Failed to request TP_IOCTL \n");
                }
        }
        gpio_direction_output(TP_IOCTL, 0);
        mdelay(200);
	
        gpio_direction_output(TP_IOCTL, 1);
